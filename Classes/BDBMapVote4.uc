...
//-----------------------------------------------------------
class BDBMapVote4 expands Mutator config;
...
//	    !aPawn.PlayerReplicationInfo.bAdmin && // Cannot kick an admin
       ...
       )
     {
        ...
        foreach AllActors(class'BDBMapVote4.MapVoteWRI',MVWRI) // check all existing WRIs
        {
		...
		foreach AllActors(class'BDBMapVote4.BanListInfo', bl) break;
...
function Mutate(string MutateString, PlayerPawn Sender)
{
   ...
   if(left(Caps(MutateString),10) == "BDBMAPVOTE")
   {
      ...
      if(Mid(Caps(MutateString),11,6) == "STATUS")
      {
         ...
         // count MapVoteWRIs
         ObjectCount = 0;
         foreach AllActors(class'BDBMapVote4.MapVoteWRI',MVWRI)
            ObjectCount++;
...
function CheckDynIPBan(PlayerPawn Victim)
{
	...
	DIPI = Spawn(class'BDBMapVote4.DynIPInfo',Victim,,Victim.Location);
	if(DIPI==None) {
...
function OpenVoteWindow(PlayerPawn Sender)
{
   ...
   foreach AllActors(class'BDBMapVote4.MapVoteWRI',A) // check all existing WRIs
   {
      if(Sender == A.Owner)
         return;            // dont open if already open
   }

   MVWRI = Spawn(class'BDBMapVote4.MapVoteWRI',Sender,,Sender.Location);
   if(MVWRI==None)
   {
...
function SaveAccVotes(int WinningMapIndex)
{
   ...
   for(x=0;x<32;x++)
   {
      class'BDBMapVote4.BDBMapVote4'.default.AccName[x] = AccName[x];
      class'BDBMapVote4.BDBMapVote4'.default.AccVotes[x] = AccVotes[x];
   }
...
function TallyVotes(bool bForceMapSwitch)
{
   ...
   {             
	...
	NWNumVotes = 1;
      ...
      if(GameMode == 1) ServerTravelString = ServerTravelString $ "?mutator=BotPack.InstaGibDM,BDBMapVote4.BDBMapVote4";
      else if(GameMode == 2) ServerTravelString = ServerTravelString $ "?mutator=BotPack.NoRedeemer,BDBMapVote4.BDBMapVote4";
      ...
      {
         RepeatLimit++;
         class'BDBMapVote4.BDBMapVote4'.default.RepeatLimit = RepeatLimit;
         class'BDBMapVote4.BDBMapVote4'.static.StaticSaveConfig();
      }
      ...
      //**********Log
      //MVLog = spawn(class'BDBMapVote4.MapVoteLog');
      //MVLog.OpenLog();
